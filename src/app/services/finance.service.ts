...
export class FinanceService {
  ...
  generateReport(filters: FinanceFilters): Observable<FinancialReport> {
    ...
    return this.http.get<any[]>(`${this.apiUrl}/report`, { params }).pipe(
      map(resArray => {
        // Aggregate ALL rows returned by backend (a reinsurer can have multiple treaties)
        const rows = resArray ?? [];
        const totals = rows.reduce(
          (acc, row) => ({
            cededPremiums: acc.cededPremiums + this.rectify(row.cededPremiums),
            recoveries: acc.recoveries + this.rectify(row.recoveries),
            outstandingBalance: acc.outstandingBalance + this.rectify(row.outstandingBalance),
          }),
          { cededPremiums: 0, recoveries: 0, outstandingBalance: 0 }
        );
        ...
        const report: FinancialReport = {
          ...
          metrics: {
            cededPremiums: this.rectify(totals.cededPremiums),
            recoveries: this.rectify(totals.recoveries),
            outstandingBalance: this.rectify(totals.outstandingBalance),
          },
        };
