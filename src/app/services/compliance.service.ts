...
export interface ComplianceRules {
  maxPendingDays?: number;
  minUtilization?: number;          // 0.25 = 25%
  maxOutstandingPerTreaty?: number;
  maxLossRatio?: number;            // e.g., 1.2
}

@Injectable({ providedIn: 'root' })
export class ComplianceApiService {
  private base = 'http://localhost:8080/api/v1/compliance';

  constructor(private http: HttpClient, @Inject(PLATFORM_ID) private platformId: Object) {}

  /** Backend call, but wrapped to match AnalyticsData<T> your component expects */
  listIssues(filters: FinanceFilters = {}, rules: ComplianceRules = {}): Observable<AnalyticsData<ComplianceIssue[]>> {
    let params = new HttpParams();
    const from = this.asYMD(filters.from as any);
    const to   = this.asYMD(filters.to as any);
    if (from) params = params.set('from', from);
    if (to)   params = params.set('to', to);
    if (typeof rules.maxPendingDays === 'number')          params = params.set('maxPendingDays', String(rules.maxPendingDays));
    if (typeof rules.minUtilization === 'number')          params = params.set('minUtilization', String(rules.minUtilization));
    if (typeof rules.maxOutstandingPerTreaty === 'number') params = params.set('maxOutstandingPerTreaty', String(rules.maxOutstandingPerTreaty));
    if (typeof rules.maxLossRatio === 'number')            params = params.set('maxLossRatio', String(rules.maxLossRatio));

    return this.http.get<ComplianceIssue[]>(`${this.base}/issues`, { params }).pipe(
      map(list => ({
        data: list ?? [],
  ...
  exportIssuesCSV(filters: FinanceFilters = {}): void {
    const url = new URL(`${this.base}/issues/export`);
    const from = this.asYMD(filters.from as any);
    const to   = this.asYMD(filters.to as any);
    if (from) url.searchParams.set('from', from);
    if (to)   url.searchParams.set('to', to);
    if (isPlatformBrowser(this.platformId)) window.open(url.toString(), '_blank');
  }
